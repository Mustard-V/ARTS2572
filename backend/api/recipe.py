    ...
    def put(self):
        ...
        try:
            recipe_id = db.insert("Recipe") \
                .values(name=data['name'], recipeInstructions=data['instruction'], author_id=data['author_id']) \
                .execute()
        ...
        for tag in tag_list:
            tag_id = db.select_one('Tag') \
                .where(name=tag['tag'], tag_type=tag['type']) \
                .execute()
...
class SearchRecipe(Resource):
    ...
    def post(self):
        ...
        if 'ingredients' in data.keys():
            recipe_list = list(filter(lambda x: set(x['ingredients'].split(',')) >= set(data['ingredients']), recipe_list))

        # if tags list is not null
        # we then select all recipes from recipe_list where recipe.tags >= searched_tags
        if 'tags' in data.keys():
            recipe_list = list(filter(lambda x: set(x['tags'].split(',')) >= set(data['tags']), recipe_list))

        # if blacklist list is not null
        # we then select all recipes from recipes where recipe.tags join blacklist is empty
        if 'tags' in data.keys():
            recipe_list = list(filter(lambda x: set(x['tags'].split(',')).isdisjoint(set(data['blackList'])), recipe_list))
        ...
        for row in recipe_list:
            ...
            searched = db.select_one('Recipe').where(id=row['recipe_id']).execute()['searched'] + 1
            db.update("Recipe").set(searched=searched).where(id=row['recipe_id']).execute()
        db.commit()
